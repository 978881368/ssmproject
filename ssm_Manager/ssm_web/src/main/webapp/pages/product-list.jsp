						...
						<table id="dataList"
                            ...
                            <c:forEach var="procuct" items="${list}">
								<tr>
									...
									<td>${procuct.productNum}</td>
									<td>${procuct.productName}</td>
									<td>${procuct.cityName}</td>
									<td><fmt:formatDate value="${procuct.departureTime}" pattern="yyyy-MM-dd HH:mm"/> </td>

									<td><fmt:formatNumber pattern="0.00" value="${procuct.productPrice}"/></td>
									<td>${procuct.productStatus==1?'开启':'关闭'}</td>


									<td class="text-center">
										<button type="button" class="btn bg-olive btn-xs"
											onclick='location.href="/product/findById?id=${procuct.id}"'>编辑</button>
										<button type="button" class="btn bg-olive btn-xs"
